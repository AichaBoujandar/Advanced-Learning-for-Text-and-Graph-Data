...
def cnn_branch(n_filters,k_size,d_rate,my_input):
    feature_maps = Conv1D(n_filters,k_size,activation='relu')(my_input)
    branch_output = GlobalMaxPooling1D()(feature_maps)
    return Dropout(d_rate)(branch_output)
...
with open('C:/Users/Aicha BOUJANDAR/Desktop/3A/ALTEGRAD/TP3/aicha_boujandar/data/word_to_index.json', 'r') as my_file:
    word_to_index = json.load(my_file)

# invert mapping (for sanity checking, later)
index_to_word = dict((v,k) for k,v in word_to_index.items())

with open('C:/Users/Aicha BOUJANDAR/Desktop/3A/ALTEGRAD/TP3/aicha_boujandar/data/training.csv', 'r') as my_file:
    reader = csv.reader(my_file, delimiter=',')
    x_train = list(reader)

with open('C:/Users/Aicha BOUJANDAR/Desktop/3A/ALTEGRAD/TP3/aicha_boujandar/data/test.csv', 'r') as my_file:
    reader = csv.reader(my_file, delimiter=',')
    x_test = list(reader)

with open('C:/Users/Aicha BOUJANDAR/Desktop/3A/ALTEGRAD/TP3/aicha_boujandar/data/training_labels.txt', 'r') as my_file:
    y_train = my_file.read().splitlines()

with open('C:/Users/Aicha BOUJANDAR/Desktop/3A/ALTEGRAD/TP3/aicha_boujandar/data/test_labels.txt', 'r') as my_file:
...
doc_wv = Embedding(input_dim = max(index_to_word)+2 , output_dim = d, input_length = max_size)(doc_ints) ### fill the gap (add an Embedding layer) https://keras.io/layers/embeddings/ ###
...
get_doc_embedding = K.function([model.layers[0].input,K.learning_phase()],[model.layers[nb_layers-2].output])
...
doc_embs_after = get_doc_embedding([np.array(x_test[:n_plot]),0])[0]
print('plotting embeddings of first',n_plot,'documents')
visualize_doc_embeddings(doc_embs_after,['blue','red'],labels_plt,'after')
...
saliency_output = model.layers[nb_layers-1].output ### fill the gap (get the probability distribution over classes from the model) ###
